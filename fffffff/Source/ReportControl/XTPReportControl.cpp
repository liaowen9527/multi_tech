...
BOOL CXTPReportControl::SetFocusedRow(BOOL bEnsureVisible, CXTPReportRow* pRow, BOOL bControlKey)
{
	if (pRow)
	{
...
{
	return SetFocusedRow(TRUE, pRow, bControlKey);
}
...
void CXTPReportControl::EnsureVisible(CXTPReportRow* pCheckRow)
{
	...
	int nRowsPerLine = GetRowsPerLine();
	...
	for (top = nCheckIndex; top >= 0; top--)
	{
		...
		if (nHeight - nRowHeight < 0)
		{
			...
			if (IsIconView())
			{
				if (top / nRowsPerLine != nCheckIndex / nRowsPerLine)
				{
					top += nRowsPerLine;
				}
			}
...
void CXTPReportControl::OnRButtonDown(UINT nFlags, CPoint point)
{
	...
	if (pRow)
	{
		SetFocusedRow(m_bEnsureFocusedRowVisible, pRow, pRow->IsSelected());
		m_pointDrag = point;
...
//Need to press Control to keep selection under collapse - expand actions
			  if (!pRow->IsSelected()
				  || bControlKey
				  || bShiftKey)
				  bFocusChanged = SetFocusedRow(m_bEnsureFocusedRowVisible, pRow, bShiftKey, bControlKey);
...
void CXTPReportControl::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	if (pRow)
	{
		...
		if (pRow->IsSelected()
			&& !bShiftKey
			&& !bControlKey)
			SetFocusedRow(m_bEnsureFocusedRowVisible, pRow, bShiftKey, bControlKey);

		CXTPSmartPtrInternalT<CCmdTarget> ptrRowLock(pRow, TRUE);
