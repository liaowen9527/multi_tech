...
void CXTPReportView::PrintRow(CDC* pDC, CXTPReportRow* pRow, CRect rcRow, int nPreviewHeight)
{
	...
	BOOL bMergeCells = pPaintManager->m_bAllowMergeCells
		&& (wndReport.GetColumns()->GetGroupsOrder()->GetCount() == 0)
		&& !wndReport.GetReportHeader()->IsAllowColumnReorder()
		&& !wndReport.GetReportHeader()->IsAllowColumnSort()
		&& !wndReport.GetReportHeader()->IsAllowColumnRemove();
	...
	if (pRecord) // if drawing record, not group
	{
		...
		CArray<int, int> columnPos;
		columnPos.SetSize(m_nEndColumn + 1);
		...
		for (UINT nColumn = m_nStartColumn; nColumn < m_nEndColumn; nColumn++)
		{
			...
			if (pColumn && pColumn->IsVisible() && pRow->IsItemsVisible())
			{
				...
				columnPos[nColumn] = rcItem.left;

				CXTPReportRecordItem* pItem = pRecord->GetItem(pColumn);

				if (pItem->m_nMergePreviousCells && bMergeCells)
				{
					int nStart = max(m_nStartColumn, nColumn - pItem->m_nMergePreviousCells);
					rcItem.left = columnPos[nStart];
				}
