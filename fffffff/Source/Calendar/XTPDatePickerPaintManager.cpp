...
void CXTPDatePickerThemeOffice2007::SetControl(CXTPDatePickerControl*  pControl)
{
	m_Sink.UnadviseAll();

	CXTPDatePickerPaintManager::SetControl(pControl);

	if (pControl && XTPResourceImages() && XTPResourceImages()->GetConnection())
	{
		m_Sink.Advise(XTPResourceImages()->GetConnection(), XTP_NC_COMMON_RESOURCEIMAGES_CHANGED,
			&CXTPDatePickerThemeOffice2007::OnEvent_ResourceImagesChanged);
	}
}


void CXTPDatePickerThemeOffice2007::OnEvent_ResourceImagesChanged(XTP_NOTIFY_CODE Event, WPARAM wParam, LPARAM lParam)
{
