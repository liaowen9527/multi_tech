...
void CXTPCalendarTimeLineView::AdjustLayout(CDC* pDC, const CRect& rcView, BOOL bCallPostAdjustLayout)
{
	...
	int nGroups = m_arGroups.GetCount();
	for (int i = 0; i < nGroups; i++)
	{
		m_arGroups.GetAt(i)->AdjustLayout(pDC, rcGroup);

		rcGroup.top = m_arGroups.GetAt(i)->GetRect().bottom;
	}
...
void CXTPCalendarTimeLineView::Populate()
{
	...
	m_arGroups.Add(new CXTPCalendarTimeLineViewGroup(this));

	m_arGroups.GetAt(0)->Populate(ptrEvents);
...
void CXTPCalendarTimeLineView::Draw(CDC* pDC)
{
	...
	for (int i = 0; i < nGroups; i++)
	{
		CXTPCalendarTimeLineViewGroup* pGroup = m_arGroups.GetAt(i);
		if (pGroup)
		{
...
BOOL CXTPCalendarTimeLineView::GetScrollBarInfoV(SCROLLINFO* pSI)
{
	...
	int iBottom(0);
	for (int i = 0; i < nGroups; i++)
		iBottom = max(iBottom, m_arGroups.GetAt(i)->GetRect().bottom);
	...
	m_nVScrollPos = 0; //to show all if no need scrollbar
	return FALSE;
}

BOOL CXTPCalendarTimeLineView::GetScrollBarInfoH(SCROLLINFO* pSI, int* pnScrollStep)
{
...
int CXTPCalendarTimeLineView::GetTimeScaleID()
{
...
BOOL CXTPCalendarTimeLineView::HitTest(CPoint pt, XTP_CALENDAR_HITTESTINFO* pHitTest)
{
	if (pHitTest)
	{
		if (IsVScrollEnabled())
...
CRect CXTPCalendarTimeLineView::GetTooltipRect(const CPoint& ptHit, const XTP_CALENDAR_HITTESTINFO& hitInfo)
{
	CRect rcTip(0,0,0,0);
	CPoint pt(ptHit);
