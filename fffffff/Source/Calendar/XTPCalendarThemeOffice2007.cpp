...
void CXTPCalendarThemeOffice2007::SetCalendarControl(CXTPCalendarControl* pCalendar)
{
	...
	if (pCalendar && XTPResourceImages() && XTPResourceImages()->GetConnection())
	{
		m_Sink.Advise(XTPResourceImages()->GetConnection(), XTP_NC_COMMON_RESOURCEIMAGES_CHANGED,
			&CXTPCalendarThemeOffice2007::OnEvent_ResourceImagesChanged);
	}
}
...
void CXTPCalendarThemeOffice2007::CTOPrevNextEventButtons::RefreshMetrics(BOOL bRefreshChildren)
{
	...
	CXTPNotifyConnection* pConn = XTP_SAFE_GET2(GetTheme(), GetCalendarControl(), GetConnection(), NULL);
	if (pConn)
	{
		m_Sink.Advise(pConn, XTP_NC_CALENDAREVENTWASADDED,
			&CXTPCalendarThemeOffice2007::CTOPrevNextEventButtons::OnEvent_Calendar);
		m_Sink.Advise(pConn, XTP_NC_CALENDAREVENTWASDELETED,
			&CXTPCalendarThemeOffice2007::CTOPrevNextEventButtons::OnEvent_Calendar);
		m_Sink.Advise(pConn, XTP_NC_CALENDAREVENTWASCHANGED,
			&CXTPCalendarThemeOffice2007::CTOPrevNextEventButtons::OnEvent_Calendar);

		m_Sink.Advise(pConn, XTP_NC_CALENDARVIEWWASCHANGED,
			&CXTPCalendarThemeOffice2007::CTOPrevNextEventButtons::OnEvent_Calendar);
	}
...
int CXTPCalendarThemeOffice2007::CTOPrevNextEventButtons::HitTest(const CPoint* pPoint)
{
	int nHit = 0;
	if (GetPrevEventButtonPartX())
