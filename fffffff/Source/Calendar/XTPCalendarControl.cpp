...
CXTPCalendarControl::CXTPCalendarControl()
{
	...
	if (XTPGetTopLevelWndMsgNotifier())
	{
		...
		XTPGetTopLevelWndMsgNotifier()->Advise(this, XTP_WM_TIME_ZONE_CHANGED);
	}
...
void CXTPCalendarControl::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (IsToolTipVisible())
	{
		CRect rcTip = m_wndTip.GetHoverRect();
		ScreenToClient(&rcTip);

		HideToolTip();
		bStartEditing = m_wndTip.IsAdvancedMode() && rcTip.PtInRect(point) && !IsReadonlyMode();
	}
...
CXTPCalendarDayView* CXTPCalendarControl::GetDayView()
{
	if (m_pDayView == NULL)
...
CXTPCalendarWeekView* CXTPCalendarControl::GetWeekView()
{
	if (m_pWeekView == NULL)
...
CXTPCalendarMonthView* CXTPCalendarControl::GetMonthView()
{
	if (m_pMonthView == NULL)
		m_pMonthView = new CXTPCalendarMonthView(this);

	return m_pMonthView;
}

CXTPCalendarTimeLineView* CXTPCalendarControl::GetTimeLineView()
{
...
int CXTPCalendarControl::GetTimeLineScale()
{
	if (m_pTimeLineView)
...
void CXTPCalendarControl::ShowToolTip(const CString& strText, const CRect rcToolTip, CFont* pFont, BOOL bAdvanced)
{
	...
	if (!bAdvanced)
	{
		...
		CSize rcNeeded = m_wndTip.CalcToolTipRect(TRUE); //change from default FALSE (Issue 20746)
	...
	else if (m_bTimelineMode)
	{
		...
		CSize rcNeeded = m_wndTip.CalcToolTipRect(TRUE);
		rcTip.right = rcTip.left + rcNeeded.cx;
		rcTip.bottom = rcTip.top + rcNeeded.cy;
	}
	else
...
//<<>>
		CSize rcNeeded = m_wndTip.CalcToolTipRect(FALSE);
		rcTip.right = rcTip.left + rcNeeded.cx;
		rcTip.bottom = rcTip.top + rcNeeded.cy;
	}
...
//<<MULTI-MONITOR case fix>>
	...
	m_wndTip.Activate(bShow, bAdvanced);
...
LRESULT CXTPCalendarControl::OnTimeZoneChanged(WPARAM, LPARAM)
{
	...
	CXTPCalendarResources* pResByDP = m_pResourcesNf->GetResourcesGroupedByDP();
	ASSERT(pResByDP);
	if (!pResByDP)
		return 0;
...
BOOL CXTPCalendarControl::IsReadonlyMode()
{
