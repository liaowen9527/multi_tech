...
CXTPChartDeviceCommand* CXTPChartBubbleSeriesPointView::CreateDeviceCommand(CXTPChartDeviceContext* pDC)
{
	...
	CXTPChartAxisView* pAxisView = pSeriesView->GetAxisViewX();

	double dSize;
	double dValue = m_pPoint->GetValue(1);

	if (pStyle->GetMinSize() < CXTPChartMathUtils::EPS && pSeriesView->m_dMaxValue > CXTPChartMathUtils::EPS)
	{
		dSize = dValue * pStyle->GetMaxSize() / pSeriesView->m_dMaxValue;
	}
	else if (fabs(pSeriesView->m_dMaxValue - pSeriesView->m_dMinValue) < CXTPChartMathUtils::EPS)
	{
	...
	if (fabs(dSize) < CXTPChartMathUtils::EPS) dSize = 0.5;
