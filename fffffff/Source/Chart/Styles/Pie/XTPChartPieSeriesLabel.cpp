...
CXTPChartPointF CXTPChartLabelsView::CalculateOffset(double /*lineAngle*/, const CXTPChartRectF& rc)
{
	...
	while (true)
	{
		...
		for (int i = 0; i < m_arrLabels.GetSize(); i++)
		{
			...
			changed = true;
			top1 = float(m_arrLabels[i].GetBottom() + CXTPChartMathUtils::EPS);
			break;
		}
	...
	if (fabs(top1 - rc.GetTop()) < CXTPChartMathUtils::EPS)
		return CXTPChartPointF(0, 0);
	...
	while (true)
	{
		...
		for (int i = 0; i < m_arrLabels.GetSize(); i++)
		{
			...
			changed = true;
			top2 = float(m_arrLabels[i].GetTop() - height - CXTPChartMathUtils::EPS);
			break;
		}
