...
BOOL CXTPChartAxisViewTickEnumerator::GetNext(double& dMark)
{
	...
	{
		int nCount = m_pView->GetAxis()->GetCustomLabels()->GetCount();
		while (m_nCurCustomLabel < nCount)
		{
			...
			if (dMark >= m_dMinValue && dMark <= m_dMaxValue)
				return TRUE;
		}
...
BOOL CXTPChartAxisView::IsScollBarVisible() const
{
	...
	if (fabs(GetViewRangeMinValue() - GetRangeMinValue()) < CXTPChartMathUtils::EPS &&
		fabs(GetViewRangeMaxValue() - GetRangeMaxValue()) < CXTPChartMathUtils::EPS)
		return FALSE;
...
void CXTPChartAxisView::CreateTickMarks(CXTPChartDeviceContext* pDC, CRect rcDiagram)
{
	...
	if (m_pAxis->GetCustomLabels()->GetCount() > 0)
	{
		...
		for (int i = 0; i < nCount; i++)
		{
			...
			if (tick.m_dValue >= dMinValue && tick.m_dValue <= dMaxValue)
			{
	...
	else
	{
		...
		if (dMark < dMinValue)
			dMark += dGridSpacing;
		while (dMark < dMaxValue + CXTPChartMathUtils::EPS)
		{
			CXTPChartAxisViewTick tick;
			tick.m_dValue = dMark;
