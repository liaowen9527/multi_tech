...
, m_pParentWnd(NULL)
{
	...
	m_pImmWrapper = new CXTPImmWrapper();
...
{
	if (m_bViewOnly && !m_bAllowExpandCollapse)
		return;
...
//          nNewRow, nNewCol, nNewDispRow, nNewDispCol);
	...
	if (!m_bViewOnly || (m_bViewOnly && m_bAllowExpandCollapse))
	{
...
//  if (!m_Selection.IsSelExist() || !m_Selection.IsInSel_str(iRow, iCol))
	if (!m_Selection.IsSelExist()
		|| !m_Selection.IsInSel_str(iRow, iCol-1))
...
//      TRACE(_T("SCROLL.X. from %d -> %d \n"), (int)m_DrawTextProcessor.GetScrollXOffset(), nCurrPos);
...
void CXTPSyntaxEditCtrl::CopyOrMoveText(BOOL bCopy)
{
	...
	BOOL bBlockSelectionMode = m_Selection.bBlockSelectionMode;

	if (!bCopy)
	{
		if (!(lcDropPos_disp < m_Selection.GetNormalStart_disp()))
		{
	...
	m_Selection.Reset_str(lcDropPos_disp.nLine, nAbsDropCol - 1);
	m_Selection.SetEnd_str(finalLC.nLine, finalLC.nCol - 1);
	m_Selection.bBlockSelectionMode = bBlockSelectionMode;

	int nRowFrom = m_Selection.GetNormalStart_str().nLine;
	int nRowTo = m_Selection.GetNormalEnd_str().nLine;
	//**----------------------
	OnEditChanged(nRowFrom, m_Selection.GetNormalStart_str().nCol + 1,
		nRowTo, m_Selection.GetNormalEnd_str().nCol + 1, xtpEditActInsert);
	//**----------------------

	CalculateEditbarLength();

	SetCurCaretPos(m_Selection.GetEnd_disp().nLine, m_Selection.GetEnd_disp().nCol);

	SetDocModified();

	//**----------------------
//  OnEditChanged(nRowFrom, m_Selection.GetNormalStart_str().nCol,
//                nRowTo, m_Selection.GetNormalEnd_str().nCol, xtpEditActInsert);
	//**----------------------
...
const CXTPSyntaxEditCtrl& CXTPSyntaxEditCtrl::operator=(const CXTPSyntaxEditCtrl& src)
{
	if (::IsWindow(src.m_hWnd))
	{
		...
		SetCollapsibleNodes(src.m_bDrawNodes);
		...
		EnableVirtualSpace(src.m_bVirtualSpace);

		RecalcScrollBars();

		SetScrollPos(SB_HORZ, 0);
		m_DrawTextProcessor.SetScrollXOffset(0);
	}
...
DWORD CXTPSyntaxEditCtrl::GetAutoscrollDirection(int* pnCols, int* pnRows)
{
	...
	CRect rcText = m_DrawTextProcessor.GetTextRect();
	int nAutoZoneY = min(m_DrawTextProcessor.GetRowHeight(), rcText.Height()/4);
	int nAutoZoneX2 = min(m_DrawTextProcessor.GetTextMetrics().tmAveCharWidth, rcText.Width()/4);
...
void CXTPSyntaxEditCtrl::OnTimer(UINT_PTR nIDEvent)
{
	...
	else if (nIDEvent == TIMER_SELECTION_ID)
	{
		...
		int nNewRow = 0, nNewCol = 0, nNewDispRow = 0, nNewDispCol = 0;
		BOOL bVirtualSpace = m_Selection.IsbBlockSelectionMode() ? TRUE : -1;
		...
		if (bSelFromLeftBar && m_Selection.nSelStartTextRowFromLeftBar)
		{
			if (nNewRow >= m_Selection.nSelStartTextRowFromLeftBar)
			{
				nNewRow++;
				m_Selection.SetStart_str(m_Selection.nSelStartTextRowFromLeftBar, 0);
			}
			else
			{
				m_Selection.SetStart_str(m_Selection.nSelStartTextRowFromLeftBar+1, 0);
			}
		}

		int nEndRow_prev = m_Selection.GetEnd_disp().nLine;
		int nEndDispCol_prev = m_Selection.GetEnd_disp().nCol;
...
//                  nNewRow, nNewCol, nNewDispRow, nNewDispCol);
			...
			if (m_Selection.bWordSelectionMode)
			{
				...
				XTP_EDIT_LINECOL& lcWordEnd = m_Selection.IsSelNormal() ? lcWord2 : lcWord1;
				lcWordEnd.nCol--;

				if (bFind && lcWordEnd.IsValidData() && lcWordEnd != m_Selection.GetEnd_str())
				{
					m_Selection.SetEnd_str(lcWordEnd.nLine, lcWordEnd.nCol);
				}
			}
			...
			int nCaretLine = m_Selection.GetEnd_disp().nLine;
			if (bSelFromLeftBar && m_Selection.GetEnd_disp() >= m_Selection.GetStart_disp())
...
BOOL CXTPSyntaxEditCtrl::Select(int nRow1, int nDispCol1, int nRow2, int nDispCol2, BOOL bRedraw)
{
	...
	m_Selection.Reset_disp(nRow1, nDispCol1);
	m_Selection.SetEnd_disp(nRow2, nDispCol2);

	SetCurrentDocumentRow(m_Selection.GetNormalEnd_disp().nLine);
	m_nDispCol = m_Selection.GetNormalEnd_disp().nCol;
	m_nCurrentCol = m_Selection.GetNormalEnd_str().nCol + 1;
...
void CXTPSyntaxEditCtrl::Unselect()
{
	if (m_Selection.IsSelExist())
	{
		m_Selection.Reset_disp(m_Selection.GetEnd_disp().nLine,
							   m_Selection.GetEnd_disp().nCol);
		Invalidate(FALSE);
	}
}
...
BOOL CXTPSyntaxEditCtrl::ReplaceSel(LPCTSTR szNewText)
{
	...
	{
		SetCurrentDocumentRow(m_Selection.GetNormalStart_disp().nLine);
		m_nDispCol = m_Selection.GetNormalStart_disp().nCol;
		m_nCurrentCol = m_Selection.GetNormalStart_str().nCol + 1; //CalcAbsCol(GetLineText(m_Selection.GetNormalStart_disp().nLine), m_nDispCol);
	}
		...
		BOOL bRedraw, int nStartRow, int nStartCol)
{
	...
	if (m_Selection.IsSelExist())
	{
		if (bSearchDown)
		{
			iStartCol = m_Selection.GetNormalEnd_str().nCol + 1;  // m_ptEndSel.x;
			iStartRow = m_Selection.GetNormalEnd_str().nLine; // m_ptEndSel.y;
		}
		else
		{
			iStartCol = m_Selection.GetNormalStart_str().nCol + 1;  // m_ptStartSel.x;
			iStartRow = m_Selection.GetNormalStart_str().nLine; // m_ptStartSel.y;
		}
	}
	...
	if (bResult)
	{
		if (m_Selection.GetStart_str().nLine > nBottomRow)
		{
			ShiftCurrentVisibleRowDown(2 * GetRowPerPage() / 3);
		}
		else if (m_Selection.GetStart_str().nLine < nTopRow)
		{
			ShiftCurrentVisibleRowUp(GetRowPerPage() / 3);
		}

		SetCurCaretPos(m_Selection.GetNormalEnd_disp().nLine, m_Selection.GetNormalEnd_disp().nCol);
	}
	...
	if (bSearchForward)
	{
		for (int i = nStartPos; i <= (nLen - nSearchLen + 1); i++)
		{
			...
			if (bMatched)
			{
				...
				SetCurCaretPos(m_Selection.GetNormalEnd_disp().nLine,
					m_Selection.GetNormalEnd_disp().nCol, FALSE/*, FALSE*/);
...
void CXTPSyntaxEditCtrl::SelectWord(CPoint point)
{
	...
	if (bFind && lcWord1.IsValidData() && lcWord2.IsValidData() && !bOverSpace)
	{
		...
		SetCurCaretPos(m_Selection.GetEnd_disp().nLine,
					   m_Selection.GetEnd_disp().nCol);
...
void CXTPSyntaxEditCtrl::DeleteSelectedLines(int iForceDeleteRow)
{
	...
	if (m_Selection.IsSelExist())
	{
		iLineFrom = m_Selection.GetNormalStart_str().nLine;
		iLineTo = m_Selection.GetNormalEnd_str().nLine;;
	}
	else
	{
	...
	m_Selection.Reset_disp(iLineFrom, 1);
...
{
	if (!m_Selection.IsSelExist() ||
		(m_Selection.GetStart_disp().nLine == m_Selection.GetEnd_disp().nLine &&
		m_Selection.GetStart_disp().nLine < GetRowCount()))
	{
	...
	int iRowFrom = m_Selection.GetNormalStart_str().nLine;
	int iRowTo = m_Selection.GetNormalEnd_str().nLine;
	int iColTo = m_Selection.GetNormalEnd_str().nCol;
	int iColTo2 = m_Selection.GetStart_disp().nCol;
	...
	// Select the lines
	m_Selection.Reset_disp(iRowFrom, 1);
	m_Selection.SetEnd_disp(iRowTo + 1, 1);
...
void CXTPSyntaxEditCtrl::_RecalcHScrollMaxWidth()
{
	...
	int nMaxX = m_DrawTextProcessor.GetRowsMaxWidth();
	nMaxX += max(nPageX/4, m_DrawTextProcessor.GetTextMetrics().tmAveCharWidth * 20);
...
void CXTPSyntaxEditCtrl::_RecalcScrollBars()
{
	...
	int nPageX = rcText.Width();
	int nPageY = m_DrawTextProcessor.GetRowsCount(FALSE);
...
void CXTPSyntaxEditCtrl::RecalcHorzScrollPos()
{
	...
	si.nPage = m_DrawTextProcessor.GetTextRect().Width();
	si.nPos = nPos;
...
{
	SetValueBool(XTP_EDIT_REG_VIRTUALSPACE, bEnable, m_bVirtualSpace, bUpdateReg);
}

void CXTPSyntaxEditCtrl::SetCollapsibleNodes(BOOL bDrawNodes, BOOL bUpdateReg)
{
	SetValueBool(XTP_EDIT_REG_COLLAPSIBLENODES, bDrawNodes, m_bDrawNodes, bUpdateReg);
...
BOOL CXTPSyntaxEditCtrl::SetTabSize(int nTabSize, BOOL bUpdateReg)
{
	...
	m_DrawTextProcessor.SetTabSize(nTabSize);
...
LRESULT CXTPSyntaxEditCtrl::NotifyMouseEvent(UINT uCode, UINT nFlags, const CPoint& point)
{
...
BOOL CXTPSyntaxEditCtrl::DoChangeSelectionCase(BOOL bUpper)
{
	...
	int nRowStrat = m_Selection.GetNormalStart_disp().nLine;
	int nRowEnd = m_Selection.GetNormalEnd_disp().nLine;
	for (int nRow = nRowStrat; nRow <= nRowEnd; nRow++)
	{
		int nVisFrom = 0;
		int nVisTo = INT_MAX;

		if (m_Selection.bBlockSelectionMode || nRow == nRowStrat)
			nVisFrom = m_Selection.GetNormalStart_disp().nCol;

		if (m_Selection.bBlockSelectionMode || nRow == nRowEnd)
			nVisTo = m_Selection.GetNormalEnd_disp().nCol;
...
void CXTPSyntaxEditCtrl::CalculateEditbarLength(CDC* pDC)
{
	...
	if (GetSelMargin() || GetLineNumbers() || m_bDrawNodes)
...
void CXTPSyntaxEditCtrl::GoToRow(int iRow, BOOL bSelectRow/* = FALSE*/)
{
	...
	if (bSelectRow)
	{
		m_Selection.Reset_disp(iRow, 1);
		m_Selection.SetEnd_disp(iRow+1, 1);
...
{
	return m_DrawTextProcessor.GetRowsCount(FALSE);
}
...
CString CXTPSyntaxEditCtrl::GetLineText(int nRow, BOOL bAddCRLF, int iCRLFStyle)
{
	CString strResult;
...
BOOL CXTPSyntaxEditCtrl::PreTranslateMessage(MSG* pMsg)
{
	...
	else if (m_bEnableEditAccelerators &&
...
void CXTPSyntaxEditCtrl::GetCollapsableBlocksInfo(int nTextRow,
		CXTPSyntaxEditRowsBlockArray& rarCoBlocks)
{
