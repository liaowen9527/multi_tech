...
void CXTPTabPaintManager::CColorSetVisualStudio2010::RefreshMetrics()
{
	...
	m_clrHeaderFace.SetStandardValue(RGB(41, 56, 82));

	m_clrAutoHideFace.SetStandardValue(RGB(41, 56, 82));
...
COLORREF CXTPTabPaintManager::CColorSetVisualStudio2010::FillPropertyButton(CDC* pDC, CRect rcItem, CXTPTabManagerItem* pItem)
{
	...
	{
		if (m_bLunaTheme)
			GradientFill(pDC, rcItem, RGB(255, 252, 242), m_clrButtonSelected, pItem->GetTabManager()->GetPosition());
		else
	...
	{
		pDC->FillSolidRect(rcItem, m_clrButtonHighlighted);
		clrItem = m_clrButtonHighlighted;
	}
	else
	{
