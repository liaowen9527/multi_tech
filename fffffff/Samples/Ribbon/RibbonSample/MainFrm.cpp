...
#include "Ribbon/XTPRibbonBackstageView.h"
...
BEGIN_MESSAGE_MAP(CMainFrame, CXTPFrameWnd)
	...
	ON_COMMAND_RANGE(ID_OPTIONS_STYLEBLUE2007, ID_OPTIONS_STYLEBLACK2010, OnOptionsStyle)
	ON_UPDATE_COMMAND_UI_RANGE(ID_OPTIONS_STYLEBLUE2007, ID_OPTIONS_STYLEBLACK2010, OnUpdateOptionsStyle)
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	OnOptionsStyle(ID_OPTIONS_STYLEBLUE2010);
	return 0;
}
...
void CMainFrame::CreateDockingPane()
{
	...
	CXTPDockingPane* pwndPane1 = m_paneManager.CreatePane(
		300, CRect(0, 0,200, 120), xtpPaneDockLeft);
...
BOOL CMainFrame::CreateStatusBar()
{
	...
	pPane->SetButton();
	pPane->SetPadding(4, 0, 4, 0);
	pPane->SetIconIndex(ID_INDICATOR_MACRORECORDING);
	pPane->SetBeginGroup(FALSE);
	...
	m_wndStatusBar.GetImageManager()->SetIcons(IDB_STATUSBAR_VIEWSWITCH, switches, sizeof(switches)/sizeof(UINT), 0);
...
int CMainFrame::OnCreateControl(LPCREATECONTROLSTRUCT lpCreateControl)
{
	...
	if (lpCreateControl->nID >= ID_OPTIONS_STYLEBLUE2007 && lpCreateControl->nID <= ID_OPTIONS_STYLESYSTEM)
	{		
...
void CMainFrame::OnOptionsStyle(UINT nStyle)
{
	...
	m_paneStyles.RefreshMetrics(TRUE);
	m_paneManager.RedrawPanes();
	m_paneStyles.Invalidate();
...
void CMainFrame::OnOptionsSystem()
{
	...
	m_paneManager.GetPaintManager()->RefreshMetrics();
	m_paneStyles.RefreshMetrics(FALSE);
	m_paneManager.RedrawPanes();
	m_paneStyles.Invalidate();
