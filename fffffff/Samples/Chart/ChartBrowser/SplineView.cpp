// SplineView.cpp : implementation file
//
...
IMPLEMENT_DYNCREATE(CSplineView, CBaseView)

CSplineView::CSplineView()
	: CBaseView(CSplineView::IDD)
{
	m_strCaption = _T("Spline Style");
...
void CSplineView::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CSplineView, CBaseView)
...
void CSplineView::SetAxisTitle(CXTPChartDiagram2D* pDiagram, CString strAxisX, CString strAxisY)
{
	if (pDiagram)
	{
...
void CSplineView::CreateChart()
{
	...
	if (pCollection)
	{
		...
		if (pSeries1)
		{
			...
			pSeries1->SetStyle(new CXTPChartSplineSeriesStyle());
		}
		...
		if (pSeries2)
		{
			...
			pSeries2->SetStyle(new CXTPChartSplineSeriesStyle());
		}
		...
		if (pSeries3)
		{
			...
			pSeries3->SetStyle(new CXTPChartSplineSeriesStyle());
		}
		...
		if (pSeries4)
		{
			...
			pSeries4->SetStyle(new CXTPChartSplineSeriesStyle());
		}
	}
...
void CSplineView::OnCheckShowLabels() 
{
	UpdateData();
...
void CSplineView::OnCheckShowMarkers() 
{
	UpdateData();
...
void CSplineView::OnCbnSelchangeComboMarkerSize()
{
	UpdateData();
...
void CSplineView::OnCbnSelchangeComboMarkerType()
{
	UpdateData();
