...
BEGIN_MESSAGE_MAP(CCommandBarsDesignerApp, CWinApp)
	...
	ON_COMMAND(ID_FILE_NEWBLANK, OnFileNewBlank)
...
BOOL CCommandBarsDesignerApp::InitInstance()
{
	...
	BOOL bUserHasPermissions = ::RegSetValue(HKEY_CLASSES_ROOT, _T("CommandBarsDesigner.Document"), REG_SZ,
		_T("Comman Document"), 15 * sizeof(TCHAR)) != ERROR_ACCESS_DENIED;
...
void CCommandBarsDesignerApp::OnFileNewBlank()
{
	m_bNewBlank = TRUE;
	OnFileNew();
