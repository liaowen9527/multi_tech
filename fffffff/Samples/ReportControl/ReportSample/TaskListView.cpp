...
int CTaskListView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	CXTPReportColumn* pColumnPercent = wndReport.AddColumn(new CXTPReportColumn(COLUMN_COMPLETE, _T("% Complete"), 100));
	...
	pColumnImportance->GetEditOptions()->AddComboButton(TRUE);

	pColumnDueDate->GetEditOptions()->AddComboButton(TRUE);
