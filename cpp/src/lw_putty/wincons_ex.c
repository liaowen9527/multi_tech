                        ...
                        void (*callback)(void *ctx, int result), void *ctx)
{
    ...
    if (ret == 2) {		       /* key was different */
		if (console_batch_mode) {
			char message[2048];
			sprintf(message, wrongmsg_batch, keytype, fingerprint);
			nb_putty_printf(frontend, -1, message);
			return 0;
		}

		char message[2048];
		sprintf(message, wrongmsg, keytype, fingerprint);
		ask_input(frontend, message, line, 32);
		//fprintf(-1, wrongmsg, keytype, fingerprint);
		fflush(stderr);
	...
	if (ret == 1) {		       /* key was absent */
		if (console_batch_mode) {
			char message[2048];
			sprintf(message, absentmsg_batch, keytype, fingerprint);
			nb_putty_printf(frontend, -1, message);
			return 0;
		}

		char message[2048];
		sprintf(message, absentmsg, keytype, fingerprint);
		ask_input(frontend, message, line, 32);
		//fprintf(-1, absentmsg, keytype, fingerprint);
		fflush(stderr);
    }
	...
	if (line[0] == 'y' || line[0] == 'Y')
		...
		nb_putty_printf(frontend, -1, abandoned);
        return 0;
    }
}
	   ...
	   void (*callback)(void *ctx, int result), void *ctx)
{
	...
	char message[2048];
	sprintf(message, msg, algtype, algname);
          ...
          void (*callback)(void *ctx, int result), void *ctx)
{
	...
	char message[2048];
	sprintf(message, msg, algname, betteralgs);
