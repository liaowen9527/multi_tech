...
namespace lw_live {
	...
	void DisplayBuffer::ReadLine(size_t nRow, std::wstring& strLine, bool bCrLf /*= true*/)
	{
		std::lock_guard<std::mutex> lck(m_mutex);
		if (nRow < m_buffer.size())
		{
			VtLine& line = m_buffer[nRow];
			strLine = line.str;
			if (bCrLf)
			{
				strLine.append(m_strEol);
			}
		}
	}
