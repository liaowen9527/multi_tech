...
public:
	 ...
	 */
	RedisHash(RedisClient* conn);

	/**
	 * see redis_command::redis_command(redis_client_cluster*, size_t)
	 */
	RedisHash(RedisClientCluster* cluster, size_t max_conns = 0);

	virtual ~RedisHash(void);
	 ...
	 */
	bool hmset(const char* key, const std::map<std::string, std::string>& attrs);
	bool hmset(const char* key, const std::vector<std::string>& names,
	 ...
	 *     the given fileds
	 */
	bool hmget(const char* key, const std::vector<std::string>& names);
	bool hmget(const char* key, const std::vector<std::string>& names, std::vector<std::string>& result);
	bool hmget(const char* key, const char* names[], const size_t lens[], size_t argc, std::vector<std::string>* result = NULL);
	 ...
	 */
	int hset(const char* key, const std::string& name, const std::string& value);
	 ...
	 * -1 -- error happend or the key is not a Hash type
	 */
	int hsetnx(const char* key, const std::string& name, const std::string& value);
	 ...
	 */
	bool hget(const char* key, const std::string& name, std::string& result);
	 ...
	 * @param key {const char*} key 键值
	 *  the hash key
	 * @param result {std::map<std::string, std::string>&} 存储域字段名-值查询结果集
	 ...
	 */
	bool hgetall(const char* key, std::map<std::string, std::string>& result);
	bool hgetall(const char* key, std::vector<std::string>& names, std::vector<std::string>& values);
	...
	int hdel(const char* key, const std::vector<std::string>& names);
	int hdel(const char* key, const std::vector<const char*>& names);
	
	int hdel_fields(const char* key, const std::vector<std::string>& names);
	 ...
	 * @param key {const char*} key 键值
	 *  the hash key
	 * @param names {std::vector<std::string>&} 存储该 key 对象所有域字段名称
	 ...
	 */
	bool hkeys(const char* key, std::vector<std::string>& names);
	 ...
	 */
	int hscan(const char* key, int cursor, std::map<std::string, std::string>& out,
		const char* pattern = NULL, const size_t* count = NULL);

protected:
	void init_redis_hash(acl::redis_hash* cmd);

protected:
	RedisClient* m_pClient;
	RedisClientCluster* m_pCluster;
	int m_max_conns;
};