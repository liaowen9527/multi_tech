...
public:
	 ...
	 */
	RedisList(RedisClient* conn);

	/**
	 * see redis_command::redis_command(redis_client_cluster*, size_t)
	 */
	RedisList(RedisClientCluster* cluster, size_t max_conns = 0);

	virtual ~RedisList(void);
	 ...
	 * @param result {std::pair<std::string, std::string>&} 存储结果元素对象，该对象的
	 ...
	 */
	bool blpop(const std::vector<std::string>& keys, size_t timeout, std::pair<std::string, std::string>& result);
	 ...
	 * @see blpop
	 */
	bool brpop(const std::vector<std::string>& keys, size_t timeout, std::pair<std::string, std::string>& result);
	 ...
	 * @see rpoplpush
	 */
	bool brpoplpush(const char* src, const char* dst, size_t timeout, std::string* buf = NULL);
	 ...
	 */
	bool lindex(const char* key, size_t idx, std::string& buf);
	 ...
	 */
	int linsert_before(const char* key, const std::string& pivot, const std::string& value);
	 ...
	 */
	int linsert_after(const char* key, const std::string& pivot, const std::string& value);
	 ...
	 */
	int lpop(const char* key, std::string& buf);
	 ...
	 */
	int lpushx(const char* key, const std::string& value);
	 ...
	 * @param result {std::vector<std::string>*} 非空时存储列表对象中指定区间的元素集合
	 ...
	 */
	int lrem(const char* key, int count, const std::string& value);
	 ...
	 */
	bool lset(const char* key, int idx, const std::string& value);
	 ...
	 *      by the key, or the list specified by key is empty
	 */
	int rpop(const char* key, std::string& buf);
	 ...
	 */
	bool rpoplpush(const char* src, const char* dst, std::string* buf = NULL);
	 ...
	 */
	int rpushx(const char* key, const std::string& value);

protected:
	void init_redis_list(acl::redis_list* cmd);
