 ...
 */
class LWUtil3rd_API RedisString
{
public:
	/**
	 ...
	 */
	RedisString(RedisClient* conn);

	/**
	 * see redis_command::redis_command(redis_client_cluster*, size_t)
	 */
	RedisString(RedisClientCluster* cluster, size_t max_conns = 0);
	virtual ~RedisString(void);

	/////////////////////////////////////////////////////////////////////

	template<class T>
	static void to_value(const T& value, std::string& result)
	{
		std::stringstream ss;
		ss << value;
	 ...
	 *  or the object specified by the key is not a string
	 */
	bool psetex(const std::string& key, const std::string& value, int timeout);
	 ...
	 */
	bool get(const std::string& key, std::string& buf);
	 ...
	 */
	bool getset(const std::string& key, const std::string& value, std::string& buf);
	 ...
	 */
	int get_strlen(const std::string& key);
	 ...
	 */
	int setrange(const std::string& key, const std::string& value, unsigned offset);
	 ...
	 */
	bool getrange(const std::string& key, std::string& value, int start, int end);
	 ...
	 */
	bool setbit_(const std::string& key, unsigned offset, bool bit);
	 ...
	 */
	bool getbit(const std::string& key, unsigned offset, int& bit);
	 ...
	 *  a string
	 */
	int bitcount(const std::string& key);
	 ...
	 */
	int bitop_and(const char* destkey, const std::vector<std::string>& keys);
	 ...
	 */
	int bitop_or(const char* destkey, const std::vector<std::string>& keys);
	 ...
	 *  the size of the string stored in the destination key
	 */
	int bitop_xor(const char* destkey, const std::vector<std::string>& keys);
	 ...
	 *  if the command was executed correctly
	 */
	bool mset(const std::map<std::string, std::string>& objs);
	 ...
	 *     add ok.
	 */
	int msetnx(const std::map<std::string, std::string>& objs);
	int msetnx(const std::vector<std::string>& keys, const std::vector<std::string>& values);
	 ...
	 * @param keys {const std::vector<std::string>&} 字符串 key 集合
	 *  the given keys
...
protected:
	void init_redis_string(acl::redis_string* cmd);
