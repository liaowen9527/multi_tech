...
public:
	 ...
	 */
	RedisSet(RedisClient* conn);

	/**
	 * see redis_command::redis_command(redis_client_cluster*, size_t)
	 */
	RedisSet(RedisClientCluster* cluster, size_t max_conns = 0);

	virtual ~RedisSet(void);
	 ...
	 */
	int sadd(const char* key, const std::vector<std::string>& members);
	 ...
	 */
	bool spop(const char* key, std::string& buf);
	 ...
	 */
	int smembers(const char* key, std::vector<std::string>* members);
	 ...
	 *      the destination set
	 */
	int smove(const char* src, const char* dst, const std::string& member);
	 ...
	 *     element from one of redis_result array by argv_to_string
	 */
	int sdiff(const std::vector<std::string>& keys, std::vector<std::string>* members);
	 ...
	 */
	int sinter(const std::vector<std::string>& keys, std::vector<std::string>* members);
	 ...
	 *  a set by the key.
	 */
	int sunion(const std::vector<std::string>& keys, std::vector<std::string>* members);
	 ...
	 *  a set by the key.
	 */
	int sdiffstore(const char* dst, const std::vector<std::string>& keys);
	 ...
	 *  a set by the key.
	 */
	int sinterstore(const char* dst, const std::vector<std::string>& keys);
	 ...
	 *  a set by the key.
	 */
	int sunionstore(const char* dst, const std::vector<std::string>& keys);
	 ...
	 */
	bool sismember(const char* key, const std::string& member);
	 ...
	 *  -1 if error happened.
	 */
	int srandmember(const char* key, std::string& out);
	int srandmember(const char* key, size_t n, std::vector<std::string>& out);
	 ...
	 */
	int srem(const char* key, const std::vector<std::string>& members);
	 ...
	 * @param out {std::vector<std::string>&} 存储结果集，内部以追加方式将本次遍历
	 ...
	 */
	int sscan(const char* key, int cursor, std::vector<std::string>& out,
		const char* pattern = NULL, const size_t* count = NULL);

protected:
	void init_redis_set(acl::redis_set* cmd);
